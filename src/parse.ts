...
import type { ReviewFile, ReviewPreview, ReviewRuleGroup, ReviewRules } from './types.ts'
...
function toString(value: unknown, what: string): string {
  if (typeof value === 'string') return value
  return fail(`${what} (expected string)`, value)
}

function toFile(value: unknown, what: string): ReviewFile {
  ...
  if (typeof value.exclude_reason === 'string') file.exclude_reason = value.exclude_reason
  return file
}
...
export function parsePreview(text: string): ReviewPreview {
  const raw = parseJson(text, 'delegate preview output')
  if (!isRecord(raw)) return fail('delegate preview output (expected object)', raw)
  if (typeof raw.schema_version !== 'string') return fail('delegate preview schema_version', raw.schema_version)
  if (typeof raw.mode !== 'string' || !KNOWN_MODES.has(raw.mode)) return fail('delegate preview mode', raw.mode)
  if (!Array.isArray(raw.reviewable_files)) return fail('delegate preview reviewable_files', raw.reviewable_files)
  if (!Array.isArray(raw.excluded_files)) return fail('delegate preview excluded_files', raw.excluded_files)
  ...
  if (typeof raw.from === 'string') preview.from = raw.from
  if (typeof raw.to === 'string') preview.to = raw.to
  if (typeof raw.commit === 'string') preview.commit = raw.commit
  if (typeof raw.merge_base === 'string') preview.merge_base = raw.merge_base
  if (typeof raw.background === 'string') preview.background = raw.background
  return preview
}

/** Parse and validate `ocr delegate rule --format json` output. */
export function parseRules(text: string): ReviewRules {
  const raw = parseJson(text, 'delegate rule output')
  if (!isRecord(raw)) return fail('delegate rule output (expected object)', raw)
  if (typeof raw.schema_version !== 'string') return fail('delegate rule schema_version', raw.schema_version)
  if (!Array.isArray(raw.groups)) return fail('delegate rule groups', raw.groups)
  const groups: ReviewRuleGroup[] = raw.groups.map((g, i) => {
    if (!isRecord(g)) return fail(`groups[${i}]`, g)
    if (!Array.isArray(g.files)) return fail(`groups[${i}].files`, g.files)
    return {
