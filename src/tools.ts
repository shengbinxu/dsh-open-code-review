...
import { parsePreview, parseRules } from './parse.ts'
import { presentPreviewCall, presentRulesCall, renderPreview, renderRules } from './render.ts'
import { buildPreviewArgv, buildRulesArgv, resolvePreview, resolveRules } from './resolve.ts'
...
export function previewTool(deps: ToolDeps) {
  return defineTool({
    name: 'ocr_review_preview',
    description:
      'Determine which files open-code-review (ocr) would review, using deterministic file selection (no LLM). '
    ...
    output: {
      ...
      render: (_args, value) => renderPreview(value as unknown as ReviewPreview),
      presentationMeta: (_args, value) => value as unknown as JsonValue,
    },
    presentCall: (args) => presentPreviewCall(args),
    async execute(args: PreviewRequest, exec): Promise<ReviewPreview> {
...
export function rulesTool(deps: ToolDeps) {
  return defineTool({
    name: 'ocr_review_rules',
    description:
      'Resolve the review rules open-code-review (ocr) would apply to the given files, grouped by rule content (no LLM). '
    ...
    output: {
      schema: rulesSchema,
      render: (_args, value) => renderRules(value as unknown as ReviewRules),
      presentationMeta: (_args, value) => value as unknown as JsonValue,
    },
    presentCall: (args) => presentRulesCall(args),
    async execute(args: RulesRequest, exec): Promise<ReviewRules> {
